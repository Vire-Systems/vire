...
async def fetch_vire_toml(
    ...
    try:
        ...
        toml_raw_url = adapter.get_raw_url(
            remote_user, remote_reponame, branch, ".vire/vire.toml"
        )
...
async def fetch_package_json(
    ...
    try:
        ...
        packagejson_raw_url = adapter.get_raw_url(
            user=remote_user,
            repo_name=remote_reponame,
            branch=branch,
            path_name="package.json",
        )
        ...
        raise errors.RepoFileFetchError(
            error_title="While trying to fetch package.json, Vire encountered unexpected errors (Internal Error)."
        )
